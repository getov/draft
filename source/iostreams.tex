...
\tcode{charT}, then the expression \tcode{out \shl\ quoted(s, delim, escape)}
behaves as if it inserts the following characters into \tcode{out} using
character inserter function templates~(\ref{ostream.inserters.character}),
...
\tcode{escape} or \tcode{delim}, as determined by \tcode{operator==}, first
output \tcode{escape}.
\item \tcode{delim}.
\end{itemize}
...
\begin{itemize}
\item If \tcode{in} is an instance of \tcode{basic_istream} with member type
\tcode{char_type} the same as \tcode{charT}, then the expression
...
determined by \tcode{operator==}, then:
\begin{itemize}
...
\end{itemize}
\item If \tcode{out} is an instance of \tcode{basic_ostream} with member type
\tcode{char_type} the same as \tcode{charT}, then the expression
